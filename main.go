...
func messageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if strings.HasPrefix(m.Content, "!hey") {
		message := "Hey " + m.Mentions[0].Mention() + " ( ° ͜ʖ͡°)╭∩╮"
...
func guildCreate(s *discordgo.Session, event *discordgo.GuildCreate) {
	...
	for _, channel := range event.Guild.Channels {
		if channel.ID == event.Guild.ID {
			_, err := s.ChannelMessageSend(channel.ID, "Your meal is here. Greet people wit !hey <username>")
			if err != nil {
				log.Errorln("Error sending message: %v", err)
			}
			return
...
func main() {
	...
	log.Infoln("Pancreas is now running. Press CTRL-C to exit.")
